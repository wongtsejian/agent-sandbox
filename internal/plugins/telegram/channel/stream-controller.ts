 ...
 * Timeline:
 *   [user sends message]
 *   → work indicator appears (⏱ 0s)
 *   → work indicator updates (tools, thinking time)
 *   → draft shows thinking content
 *   → work indicator collapses (summary)
 *   → response sent as new message
 */
export class StreamController {
  ...
  toolStart(toolCallId: string, title: string, status?: ToolCallStatus): void {
    ...
    // Auto-close active thinking when a tool starts
    const thinking = this.lastThinking();
    if (thinking && !thinking.ended) {
      thinking.ended = true;
  ...
  pushText(text: string): void {
    if (this.done) return;
    // Silently accumulate — delivered as new message at finalize()
  ...
  async finalize(): Promise<void> {
    ...
    // Wait for any pending outbound operations
    await this.outboundQueue;

    // Collapse work indicator (if active)
  ...
  private ensureEditTimer(): void {
    ...
    // Also do an immediate first tick
    this.tickEdit();
  }
